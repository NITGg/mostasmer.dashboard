...
import { Category } from "@/app/[locale]/category/categoryData";
...
const Categories: React.FC<CategoriesProps> = ({
  ...
  const categoriesRedux = useAppSelector((state) => state.category.categories);
  ...
  const handleViewBrands = async (categoryId: number, categoryName: string) => {
    try {
      ...
      const brandIds = Array.from(
        new Set(offersData.offers.map((offer: any) => offer.brandId))
      );
      ...
      router.push(
        `/${locale}/brands?ids=${brandIds.join(
          ","
        ...
        <td className="px-6 py-4 text-right">
          <div className="flex justify-end gap-3">
            <button
              ...
              className="text-[#24ae9f] hover:text-green-700"
              aria-label={t("viewBrands")}
            >
              <BrandIcon className="w-5 h-5 text-[#24ae9f]/80 hover:text-gray-700" />
            </button>
            <button
              onClick={() => {
                setOpen(true);
