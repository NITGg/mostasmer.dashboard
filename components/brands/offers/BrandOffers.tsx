...
// //                 <td className="px-6 py-4">
// //                     <a href={offer.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
// //                         {offer.url.length > 20 ? `${offer.url.slice(0, 20)}...` : offer.url}
// //                     </a>
// //                 </td>
// //                 <td className="px-6 py-4">{offer.ratio}%</td>
// //                 <td className="px-6 py-4">{new Date(offer.validFrom).toLocaleDateString()}</td>
...
//             if (!response.ok) {
//                 const error = await response.json()
//                 throw new Error(error.message || t('brand_offerDeleteError'))
//             }
...
const BrandOffers = ({ brandId }: { brandId: string }) => {
    ...
    const fetchOfferTypeInfo = async () => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_BASE_URL}/api/offer-info/8?lang=${locale}`,
                {
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`
        ...
        {offer.id.toString().padStart(3, '0')}
                ...
                <td className="px-6 py-4 font-bold">
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-bold">
                            {mlang(offer.category.name, locale as "en" | "ar")}
                        </span>                    </div>
                </td>
                <td className="px-6 py-4">
                    <a href={offer.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div>
                            ...
                            >
                                ...
                                {availableCategories.map(category => (
                                    <option key={category.id} value={category.id}>
                                        {mlang(category.name, locale as "en" | "ar")}
                                    </option>
                                ))}
                            </select>
                            {errors.categoryId && (
