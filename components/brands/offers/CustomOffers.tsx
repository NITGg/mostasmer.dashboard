...
const CustomOffers = ({ brandId }: { brandId: string }) => {
  ...
  const fetchOfferTypeInfo = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/offer-info/10?lang=${locale}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const renderTableRows = () => {
    return offers.map((offer) => (
      <tr
        key={offer.id}
        className={`border-b font-bold odd:bg-gray-50 even:bg-white ${
          new Date(offer.validTo) < new Date()
            ? "font-extrabold line-through text-red-500"
            : ""
        }`}
      >
        ...
        <td className="px-6 py-4">
          <div className="flex space-x-2">
            ...
            >
              <TrashIconn className="w-4 h-4 text-red-500 gap-3" />
            </button>
          </div>
        </td>
      </tr>
    ...
    <div className="mt-8">
          ...
          <Pagination
            ...
            downloadButton={
              <DownloadButton
                ...
                fields={["id", "ratio", "createdAt", "brand", "userClass"]}
                model="CustomOffer"
                include={{
                  brand: { select: { name: true } },
                  userClass: { select: { name: true } },
                }}
              />
            }
          />
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={handleCloseDialog}>
        <DialogContent>
          <h2 className="text-lg font-semibold mb-4 text-center">
            {editingOffer
              ? t("custom_offer_table_edit_custom_offer")
              : t("custom_offer_table_add_new_custom_offer")}
          </h2>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {loading
                  ? t("custom_offer_table_saving")
                  : editingOffer
                  ? t("custom_offer_table_update")
                  : t("custom_offer_table_add")}
              </button>
            </div>
          </form>
        </DialogContent>
      ...
      {deleteConfirmationId && (
        ...
        >
          <DialogContent>
            <h2 className="text-lg font-semibold mb-4 text-center">
              {t("custom_offer_table_confirm_delete")}
            </h2>
            <p className="mb-4">
              {t("custom_offer_table_confirm_delete_message")}
            </p>
            <div className="flex justify-end space-x-3">
              <button
              ...
              >
                {loading
                  ? t("custom_offer_table_deleting")
                  : t("custom_offer_table_delete")}
              </button>
            </div>
          </DialogContent>
        </Dialog>
